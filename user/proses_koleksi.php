...
$cek = mysqli_query($conn, "SELECT * FROM koleksipribadi WHERE BukuID=$id");
$jumlah = mysqli_num_rows($cek);

if ($jumlah > 0) {
    echo '<div class="message-box">';
    echo 'Anda sudah menyukai buku ini';
    echo '</div>';
    echo '<button class="btn-back" onclick="goBack()">Kembali</button>';
} else {
    $insert = mysqli_query($conn, "INSERT INTO koleksipribadi (BukuID, Judul) 
VALUES ($id)");
